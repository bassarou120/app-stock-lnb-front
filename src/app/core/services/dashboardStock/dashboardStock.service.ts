...
import { InterventionsVehiculeService } from '../interventionvehicules/interventionvehicules.service';
...
getVehiculesAssurancesExpirantes(): Observable<InterventionVehicule[]> {
  return this.http.get<any>(`${this.url}/assurance-expiresoon`).pipe(
    map(response => response.assurances_expirantes)
  );
}

getVehiculesEntretenusoon(): Observable<InterventionVehicule[]> {
  return this.http.get<any>(`${this.url}/assurance-expiresoon`).pipe(
    map(response => response.reparations_recentes)
  );
}

getVisiteTechniqueSoon(): Observable<InterventionVehicule[]> {
  return this.http.get<any>(`${this.url}/assurance-expiresoon`).pipe(
    map(response => response.visites_techniques_proches)
  );
}

  CountVehiculesAssurancesExpirantes(): Observable<any> {
  return this.http.get<any>(`${this.url}/assurance-expiresoon`);
  // Supprimer le .pipe(map(...)) pour avoir la réponse complète
}
